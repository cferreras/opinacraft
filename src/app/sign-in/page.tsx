...
export default function SignInPage() {
  ...
  return <AuthShell title="Bienvenido de nuevo" description="Inicia sesión para continuar en tu cuenta de OpinaCraft." footer={<>¿No tienes una cuenta? <Link href="/sign-up" className="font-medium text-primary hover:underline">Crear una</Link></>}><form onSubmit={handleSubmit} className="grid gap-5"><Field><FieldLabel htmlFor="sign-in-email">Email</FieldLabel><Input id="sign-in-email" type="email" value={email} onChange={(event) => setEmail(event.target.value)} required autoComplete="email" /></Field><Field><FieldLabel htmlFor="sign-in-password">Contraseña</FieldLabel><Input id="sign-in-password" type="password" value={password} onChange={(event) => setPassword(event.target.value)} required autoComplete="current-password" /><Button variant="link" asChild size="sm" className="h-auto w-fit p-0"><Link href="/forgot-password">¿Has olvidado la contraseña?</Link></Button></Field>{error ? <Alert variant="destructive"><AlertDescription>{error}</AlertDescription></Alert> : null}<Button type="submit" disabled={isPending}>{isPending ? "Iniciando sesión…" : "Iniciar sesión"}</Button>{discordEnabled ? <><div className="relative py-1 text-center text-xs text-muted-foreground"><Separator /><span className="relative -top-3 bg-card px-3">o</span></div><Button type="button" variant="outline" onClick={handleDiscordSignIn}>Continuar con Discord</Button></> : null}</form></AuthShell>;
}
