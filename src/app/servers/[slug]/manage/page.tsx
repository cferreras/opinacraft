        ...
        <div className="pt-7 sm:pt-8">
          <header className="flex flex-col gap-5 border-b border-[#e7ebef] pb-6 lg:flex-row lg:items-end lg:justify-between">
            <div className="min-w-0">
              <Link href="/dashboard/servers" className="inline-flex min-h-8 items-center gap-1.5 text-[11px] font-medium text-[#68758b] transition hover:text-[#2d34cf]">
              ...
              <div className="mt-5 flex items-center gap-3.5">
                ...
                <div className="min-w-0">
                  <p className="text-[10px] font-semibold uppercase tracking-[0.13em] text-[#2d34cf]">Server workspace</p>
                  <h1 className="mt-1 truncate text-[30px] font-bold leading-none tracking-[-0.06em] text-[#101722] sm:text-[36px]">Manage {server.name}</h1>
                </div>
              </div>

              <div className="mt-4 flex flex-wrap items-center gap-2 text-[11px] text-[#667287]">
            ...
            <div className="flex shrink-0 flex-wrap items-center gap-2">
              {server.publicationStatus === "published" ? (
                <Link href={`/servers/${server.slug}`} className="inline-flex h-10 items-center gap-2 rounded-lg border border-[#cbd2ff] bg-white px-3.5 text-[11px] font-semibold text-[#2d34cf] transition hover:bg-[#f0f1ff]">
                  <IconEye aria-hidden="true" size={16} stroke={1.7} />
                  View public page
                  <IconExternalLink aria-hidden="true" size={14} stroke={1.7} />
                </Link>
              ) : (
                <span className="inline-flex h-10 items-center gap-2 rounded-lg border border-[#e0e6eb] bg-[#fafbfc] px-3.5 text-[11px] font-medium text-[#7a8698]">
                  <IconEye aria-hidden="true" size={16} stroke={1.7} />
                  Draft preview
                </span>
              )}
          ...
          <nav className="mt-5 -mx-1 flex gap-1 overflow-x-auto px-1 pb-1" aria-label="Manage server sections">
            <SectionLink href="#details" icon={<IconFileText aria-hidden="true" size={15} stroke={1.7} />}>Details</SectionLink>
            <SectionLink href="#media" icon={<IconPhoto aria-hidden="true" size={15} stroke={1.7} />}>Branding</SectionLink>
            {server.role === "owner" ? <SectionLink href="#verification" icon={<IconShieldCheck aria-hidden="true" size={15} stroke={1.7} />}>Verification</SectionLink> : null}
            {(server.role === "owner" || server.role === "admin") ? <SectionLink href="#team" icon={<IconUsers aria-hidden="true" size={15} stroke={1.7} />}>Members</SectionLink> : null}
          </nav>
          ...
          <div className="mt-5 grid gap-5 lg:grid-cols-[minmax(0,1fr)_286px] lg:items-start">
            <div className="min-w-0 space-y-5">
              <section id="details" className="scroll-mt-5 rounded-2xl border border-[#e0e6eb] bg-white p-5 shadow-[0_1px_2px_rgba(16,30,45,0.02)] sm:p-6">
  ...
  <aside className="order-first min-w-0 lg:order-none lg:sticky lg:top-5 lg:self-start">
    <div className="space-y-4">
                <section className="rounded-2xl border border-[#e0e6eb] bg-[#fbfcff] p-5" aria-labelledby="workspace-summary">
                  <p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-[#7a86a0]">Public preview</p>
                  <div className="mt-4 flex items-center gap-3">
                    <ServerLogo name={server.name} media={server.media} className="h-11 w-11 rounded-lg" />
                    <div className="min-w-0">
                      <h2 id="workspace-summary" className="truncate text-[14px] font-semibold text-[#17202a]">{server.name}</h2>
                      <p className="mt-0.5 text-[10px] text-[#7a8698]">/{server.slug}</p>
                    </div>
                  </div>
                  <div className="mt-4 rounded-lg border border-[#e2e7ec] bg-white p-3">
                    ...
                    <div className="mt-2.5 flex items-center justify-between gap-3 text-[11px]">
                      ...
                      <span className="inline-flex items-center gap-1 font-semibold text-[#0c8950]"><IconCheck aria-hidden="true" size={14} stroke={2} />{server.verificationStatus === "verified" ? "Ready" : "Pending"}</span>
                    </div>
                  </div>
                  {server.publicationStatus === "published" ? <Link href={`/servers/${server.slug}`} className="mt-3 inline-flex h-10 w-full items-center justify-between rounded-lg bg-[#3029e7] px-3.5 text-[11px] font-semibold text-white shadow-[0_5px_12px_rgba(48,41,231,0.14)] transition hover:bg-[#2821c8]">Open public page <IconChevronRight aria-hidden="true" size={16} stroke={1.8} /></Link> : <p className="mt-3 rounded-lg bg-[#fff4df] px-3 py-2.5 text-[10px] leading-4 text-[#8b641e]">Publish the listing from Server details when it is ready to be discovered.</p>}
                </section>

                <section className="rounded-2xl border border-[#e0e6eb] bg-white p-5" aria-labelledby="workspace-nav-heading">
                  <h2 id="workspace-nav-heading" className="text-[14px] font-semibold tracking-[-0.02em] text-[#17202a]">Workspace</h2>
                  <p className="mt-1.5 text-[11px] leading-5 text-[#718097]">Make changes, verify ownership and keep your team in sync.</p>
                  <div className="mt-4 grid gap-1">
                    <RailLink href="#details" icon={<IconFileText aria-hidden="true" size={16} stroke={1.7} />}>Edit listing</RailLink>
                    <RailLink href="#media" icon={<IconPhoto aria-hidden="true" size={16} stroke={1.7} />}>Manage branding</RailLink>
                    {server.role === "owner" ? <RailLink href="#verification" icon={<IconShieldCheck aria-hidden="true" size={16} stroke={1.7} />}>Verify endpoints</RailLink> : null}
                    {(server.role === "owner" || server.role === "admin") ? <RailLink href="#team" icon={<IconUsers aria-hidden="true" size={16} stroke={1.7} />}>Manage members</RailLink> : null}
                  </div>
                </section>

                <section className="rounded-2xl border border-[#e0e6eb] bg-white p-5" aria-labelledby="connection-summary-heading">
                  <h2 id="connection-summary-heading" className="text-[14px] font-semibold tracking-[-0.02em] text-[#17202a]">Connection</h2>
                  <p className="mt-1.5 text-[11px] leading-5 text-[#718097]">The addresses currently attached to this listing.</p>
                  <div className="mt-4 grid gap-2.5">
                    {server.endpoints.length ? server.endpoints.map((endpoint) => (
                      <div key={endpoint.edition} className="flex min-w-0 items-center gap-2.5 rounded-lg border border-[#e3e7ec] bg-[#fbfcff] px-3 py-2.5">
                        ...
                        <div className="min-w-0">
                          <p className="text-[10px] font-semibold capitalize text-[#35415b]">{endpoint.edition}</p>
                          <code className="mt-0.5 block truncate text-[10px] text-[#718097]">{formatEndpoint(endpoint)}</code>
                        </div>
                      </div>
                    )) : <p className="rounded-lg bg-[#f5f7f9] p-3 text-[11px] text-[#6c788b]">No connection addresses yet.</p>}
                  </div>
                </section>
              </div>
            </aside>
...
function Notice({ children, tone = "normal" }: { children: React.ReactNode; tone?: "normal" | "warning" }) {
  return <p className={`ui-notice px-3 py-2.5 text-[11px] leading-5 ${tone === "warning" ? "ui-notice-warning" : ""}`}>{children}</p>;
}
...
function PanelHeading({ eyebrow, title, description }: { eyebrow: string; title: string; description: string }) {
  return (
    <div>
      ...
      <h2 className="mt-1.5 text-[20px] font-semibold tracking-[-0.03em] text-[#101722]">{title}</h2>
      <p className="mt-1.5 max-w-[520px] text-[12px] leading-5 text-[#667287]">{description}</p>
    </div>
  );
}

function SectionLink({ href, icon, children }: { href: string; icon: React.ReactNode; children: React.ReactNode }) {
  return <a href={href} className="inline-flex h-9 shrink-0 items-center gap-2 rounded-lg border border-transparent px-3 text-[11px] font-medium text-[#68758b] transition hover:border-[#dfe4ea] hover:bg-[#fafbfc] hover:text-[#2d34cf]">{icon}{children}</a>;
}

function RailLink({ href, icon, children }: { href: string; icon: React.ReactNode; children: React.ReactNode }) {
  return <a href={href} className="group flex min-h-10 items-center gap-2.5 rounded-lg px-2.5 text-[11px] font-medium text-[#59677c] transition hover:bg-[#f1f2ff] hover:text-[#2d34cf]"><span className="text-[#7a86a0] transition group-hover:text-[#2d34cf]">{icon}</span><span className="flex-1">{children}</span><IconChevronRight aria-hidden="true" size={14} stroke={1.7} className="text-[#b0b8c5] transition group-hover:text-[#6c75dc]" /></a>;
}
