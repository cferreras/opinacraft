...
export async function moderateReportAction(formData: FormData) {
  ...
  if (!reportId || !["dismissed", "hidden", "restored"].includes(decision)) redirect("/admin?error=invalid");
  try {
    await moderateReport(session.user.id, reportId, decision as "dismissed" | "hidden" | "restored");
  } catch {
...
export async function moderateReviewReportAction(formData: FormData) {
  ...
  if (!reportId || !["dismissed", "hidden", "restored"].includes(decision)) redirect("/admin?error=invalid");
  try {
    await moderateReviewReport(session.user.id, reportId, decision as "dismissed" | "hidden" | "restored");
  } catch {
    redirect("/admin?error=forbidden");
  }
  revalidatePath("/admin");
