...
import { db } from "@/db";
import { moderationEvents, reviewReplies, serverEndpoints, serverMedia, serverMembers, serverReviewReports, serverReviews, serverGameModes, servers } from "@/schema";
...
export async function GET() {
  ...
  const gameModeRows = ids.length ? await db.select({ serverId: serverGameModes.serverId, mode: serverGameModes.mode }).from(serverGameModes).where(inArray(serverGameModes.serverId, ids)) : [];
  ...
  return NextResponse.json({ exportedAt: new Date().toISOString(), account: { id: session.user.id, email: session.user.email, name: session.user.name }, servers: owned, endpoints, media, gameModes: gameModeRows, reviews, officialReplies: replies, reviewReports, moderationEvents: moderation });
}
