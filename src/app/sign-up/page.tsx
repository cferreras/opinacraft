...
export default function SignUpPage() {
  const [name, setName] = useState(""); const [email, setEmail] = useState(""); const [password, setPassword] = useState(""); const [error, setError] = useState<string | null>(null); const [message, setMessage] = useState<string | null>(null); const [emailAlreadyRegistered, setEmailAlreadyRegistered] = useState(false); const [isPending, setIsPending] = useState(false); const discordEnabled = clientEnv.NEXT_PUBLIC_DISCORD_ENABLED === "true";
  async function handleSubmit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); setError(null); setMessage(null); setEmailAlreadyRegistered(false); setIsPending(true); const callbackURL = safeCallbackUrl(new URLSearchParams(window.location.search).get("callbackURL"), "/profile"); try { const { error: signUpError } = await authClient.signUp.email({ name, email, password, callbackURL }); if (signUpError) { const isDuplicateEmail = signUpError.code === "USER_ALREADY_EXISTS" || signUpError.code === "USER_ALREADY_EXISTS_USE_ANOTHER_EMAIL"; setEmailAlreadyRegistered(isDuplicateEmail); setError(isDuplicateEmail ? "Ya existe una cuenta con ese correo." : signUpError.message ?? "No se ha podido crear la cuenta."); return; } setMessage("Cuenta creada. Revisa tu email y confirma el enlace antes de iniciar sesión."); } catch (caughtError) { setError(caughtError instanceof Error ? caughtError.message : "No se ha podido crear la cuenta."); } finally { setIsPending(false); } }
