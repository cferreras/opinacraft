...
export function PlayerHistoryCard({ serverId, initialData, mode = "public" }: Props) {
  ...
  useEffect(() => { const updateNow = () => setNowMs(Date.now()); updateNow(); const interval = window.setInterval(updateNow, 60_000); return () => window.clearInterval(interval); }, []);
