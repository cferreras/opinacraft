...
type ServerAccessFieldsProps = {
  ...
  errors?: AccessErrors;
  onAccessChange?: (details: { accessType: ServerAccessType; accountMode: ServerAccountMode; authMode: ServerAuthMode }) => void;
};
...
export function ServerAccessFields({
  ...
  function selectAccessType(value: ServerAccessType) { setAccessType(value); onAccessChange?.({ accessType: value, accountMode: selectedProfile.accountMode, authMode: selectedProfile.authMode }); }
  function selectProfile(option: (typeof serverAccessProfiles)[number]) { setProfile(accessProfileKey(option)); onAccessChange?.({ accessType, accountMode: option.accountMode, authMode: option.authMode }); }
          ...
          {accessOptions.map((option) => {
            ...
            return (
              <label key={option.value} className={`flex cursor-pointer items-start gap-3 rounded-lg border p-4 transition-colors ${selected ? "border-primary/40 bg-primary/5" : "bg-muted/20 hover:bg-muted/40"}`}>
                <input type="radio" name="accessType" value={option.value} checked={selected} onChange={() => selectAccessType(option.value)} className="mt-0.5 size-4 accent-primary" />
