...
export async function moderateReviewReport(userId: string, reportId: string, decision: "dismissed" | "hidden" | "restored") {
  ...
  return db.transaction(async (tx) => {
    ...
    if (review && decision === "hidden" && review.status === "published") {
      await tx.update(serverReviews).set({ status: "hidden", updatedAt: new Date() }).where(eq(serverReviews.id, review.id));
    }
    if (review && decision === "restored" && review.status === "hidden") {
      await tx.update(serverReviews).set({ status: "published", updatedAt: new Date() }).where(eq(serverReviews.id, review.id));
    }
    ...
    if (review?.authorEmail && (decision === "hidden" || decision === "restored")) {
      await tx.insert(notificationJobs).values({
