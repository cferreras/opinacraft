...
async function ping(endpoint) {
  ...
  return new Promise((resolve) => {
    ...
    timer = setTimeout(() => settle({ ...endpoint, online: false, latencyMs: null }), 5000);
    socket.on("message", (message) => {
      ...
      settle({ ...endpoint, online: true, playersCurrent: Number(fields[4]) || 0, playersMax: Number(fields[5]) || 0, version: fields[3], latencyMs: Date.now() - started });
    });
    socket.on("error", () => settle({ ...endpoint, online: false, latencyMs: null }));
    socket.send(packet, endpoint.port, address, (error) => {
      if (error) settle({ ...endpoint, online: false, latencyMs: null });
    });
  });
}
...
for (let attempt = 0; attempt < 3; attempt += 1) {
  try {
    const response = await fetch(`${url}/api/internal/monitor/results`, { method: "POST", headers: { "content-type": "application/json", "x-monitor-signature": signature }, body: payload, signal: AbortSignal.timeout(10_000) });
